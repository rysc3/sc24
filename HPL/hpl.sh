...
read_net_affinity_map() {
    ...
    # @ryan debug
    echo affinity string: $affinity_string
}
...
read_rank() {
  ...
  if [ -n "${OMPI_COMM_WORLD_RANK}" ]; then
    ...
    echo "Global Rank OMPI: ${RANK}"
  elif [ -n "${PMIX_RANK}" ]; then
    RANK=${PMIX_RANK}
    echo "Global Rank PMIX: ${RANK}"
  elif [ -n "${PMI_RANK}" ]; then
    RANK=${PMI_RANK}
    echo "Global Rank PMI: ${RANK}"
  elif [ -n "${SLURM_PROCID}" ]; then
    RANK=${SLURM_PROCID}
    echo "Global Rank SLURM PROCID: ${RANK}"
  else
  ...
  if [ -n "${OMPI_COMM_WORLD_LOCAL_RANK}" ]; then
    ...
    echo "Local Rank OMPI: ${$LOCAL_RANK}"
  elif [ -n "${SLURM_LOCALID}" ]; then
    LOCAL_RANK=${SLURM_LOCALID}
    echo "Local Rank SLURM LOCALID: ${$LOCAL_RANK}"
  elif [ -n "${MPI_LOCALRANKID}" ]; then
    LOCAL_RANK=${MPI_LOCALRANKID}
    echo "Local Rank MPI LOCALRANK: ${$LOCAL_RANK}"
  else
...
if [[ -n "${NET_AFFINITY}" ]]; then
  ...
  if [ -n "${NET}" ]; then
    ...
    echo "Set ucx_net_devices to: ${UCX_NET_DEVICES}"
  fi
fi
...
if [[ -n "${GPU_AFFINITY}" ]]; then
  ...
  echo "set HPL_VISIBLE_DEVICES: ${HPL_VISIBLE_DEVICES}"
  echo "For reference, CUDA_VISIBLE_DEVICES: ${CUDA_VISIBLE_DEVICES}"
fi
...
if [[ -n "${MEM_AFFINITY}" ]]; then
  ...
  MEMBIND="--membind=${MEM}"
  echo "Set mem affinity to: ${MEMBIND}"
fi
...
if [[ -n "${CPU_AFFINITY}" ]]; then
  ...
  echo "Set cpu affinity to: ${CPU}"
fi

if [ -n "${MEMBIND}" ] || [ -n "${CPUBIND}" ]; then
  NUMCMD="numactl "
  echo "Since either MEMBIND or CPUBIND, Setting: ${NUMCMD}"
fi

if [ -z "$DAT" ]; then
  error "DAT file not provided"
  echo "Setting Dat: ${DAT}"
fi
...
echo "Final Command =>"
echo "${NUMCMD} ${CPUBIND} ${MEMBIND} ${XHPL} ${DAT}"
